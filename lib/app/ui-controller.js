...
const CSS_COLORS_WITH_DATA =
    require('../css-colors.json').filter(color => typeof color.r === 'number'),
  $ = window.jQuery;
var $colorpicker, remakeOptions = {},
  getValue, setValue, getFormats, setFormats, getPalette, setPalette, getInput;
...
function createUi(initOptions) {
  ...
  const
    ...
    FORMS = {/* eslint-disable key-spacing */
  ...
  var form = initOptions.form && FORMS[initOptions.form] ?
      initOptions.form : 'default';
  ...
  $colorpicker = $('<span id="color-picker"/>').addClass(`form-${form}`).appendTo('body')
  .colorpicker({
    alpha: true,
    closeOnEscape: false,
...
exports.init = initOptions => {
  if (!$colorpicker ||
    ...
    if ($colorpicker) {
      ...
      getValue = setValue = getFormats = setFormats = getPalette = setPalette = getInput = null;
    }
