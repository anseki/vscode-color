...
var $colorpicker, remakeOptions = {},
  getValue, setValue, getFormats, setFormats, getPalette, setPalette, getInput, formatsOrder, convert;
...
function createUi(initOptions) {
  ...
  const
    ...
    RE_ANGLE = new RegExp(`^(${PTN_NUM}) *(` +
      ANGLE_UNITS.map(unit => unit.replace(/[\x00-\x7f]/g, // eslint-disable-line no-control-regex
   ...
   * @returns {string} The normalized text. (e.g. 100grad)
   */
   ...
   * @returns {string} The normalized text. (e.g. 80%)
   */
  ...
  /**
   * @param {number} num01 - The number in the range [0, 1]. (e.g. 0.8)
   * @returns {string} The normalized text. (e.g. 0.8)
   */
   ...
   * @returns {boolean} false: A function notation is caught, but it has error (it's not another notation).
   * @returns {null} `text` is not function notation (it might be another notation).
   */
  function parseFunc(text) {
    ...
    if (/\f|\x07/.test(text)) { return false; } // eslint-disable-line no-control-regex
   ...
   * @returns {Object|null} A palette element.
   */
  function parsePaletteElement(element) {
    ...
    function asNumber(value) {
      var type;
      return value == null || (type = typeof value) === 'object' ? false :
        type === 'number' ? value :
    ...
    function asString(value) {
      return value != null && typeof value !== 'object' && (value += '') !== '' ? value : false;
    }
    ...
    .colorpicker({
      ...
      part: {
        formats: {
          ...
          bindFormatsOrder: method => { formatsOrder = method; }
        },
        swatchesswitcher: {
  ...
  initOptions.resizeWindow(
    $colorpicker.outerWidth() +
      (initOptions.disableShadow ? 0 : BODY_PADDING_LEFT + BODY_PADDING_RIGHT),
    $colorpicker.outerHeight() +
      (initOptions.disableShadow ? 0 : BODY_PADDING_TOP + BODY_PADDING_BOTTOM),
...
exports.init = initOptions => {
  if (!$colorpicker ||
    ...
    if ($colorpicker) {
      ...
      getValue = setValue = getFormats = setFormats =
        getPalette = setPalette = getInput = formatsOrder = convert = null;
    }
  ...
  if (initOptions.command === 'pick') {
    ...
    $(document.body).toggleClass('disableShadow', initOptions.disableShadow)
      .toggleClass('disableTransparent', initOptions.disableTransparent);

    $colorpicker.css('display', ''); // See commit().
    setValue(initOptions.value || initOptions.stats.value, true);
    if (initOptions.formatsOrder) { formatsOrder(initOptions.formatsOrder); }
    return null;
  } else { // convert
