 ...
 * @returns {string} Found label, or `''` if it's not specified.
 * @returns {null} The content has no label-field.
 * @returns {boolean} false: Invalid `content`.
 */
...
function parsePalette(content) {
  ...
  return (typeof label === 'string' ? content.slice(1) : content)
    .reduce((palette, element, i) => {
      const paletteElement = paletteParser(element);
      if (paletteElement) {
...
exports.init = (storeDir, parser, defaultPalette, defaultPaletteLabel) => {
  ...
  function mkdirP(path) { // mkdir -p
    path.split(/\/|\\/).reduce((parents, dir) => {
      const path = pathUtil.resolve((parents += dir + pathUtil.sep)); // normalize
      if (!isExists(path)) {
        fs.mkdirSync(path);
  ...
  function copyFiles(srcDir, destDir) {
    ...
    function copyFileSync(src, dest) {
      ...
      while (true) {
        const readSize = fs.readSync(fdSrc, buffer, 0, BUFFER_SIZE);
        if (readSize <= 0) { break; }
    ...
    fs.readdirSync(srcDir).forEach(fileName => {
      const srcPath = pathUtil.join(srcDir, fileName),
  ...
  function parseStoreDir() {
    fs.readdirSync(curStoreDir).forEach(fileName => {
      const path = pathUtil.join(curStoreDir, fileName);
      let matches, label;
